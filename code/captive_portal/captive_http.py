...
class HTTPServer(Server):
    def __init__(self, poller, local_ip):
        ...
        self.request = dict()
        self.conns = dict()
        self.routes = {b"/": b"/code/captive_portal/index.html", b"/login": self.login}
    ...
    def login(self, params):
        ...
        base_url = unquote(params.get(b"base_url", None))
        username = unquote(params.get(b"username", None))

        # Write out credentials
        Creds(ssid=ssid, password=password, base_url=base_url, username=username).write()
